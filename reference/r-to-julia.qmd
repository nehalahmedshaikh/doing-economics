...
| `spread(key = Sex, value = Value)` | `unstack(df, :Sex, :Value)` |
| `reduce(full_join, frames)` | `reduce((a, b) -> outerjoin(a, b; on = [:Country, :Year]), frames)` |
| `lag(x)` / `lead(x)` | `vcat(missing, x[1:end-1])` / `vcat(x[2:end], missing)` |
...
| R | Julia |
|---|---|
| `mFilter::hpfilter(x, freq = 6.25)` | solve `(I + λD′D) * trend = x`, then `cycle = x - trend` |

There is no HP-filter package in this project environment. Project 13 implements the standard
penalized least-squares definition directly: `D` is the second-difference matrix and λ is 6.25
for annual data. The local helper first checks that years are contiguous; silently filtering a
series with gaps would pretend irregular observations were equally spaced.
